...
Shader "Hidden/EdgeDetect-PostProcess"
{
	HLSLINCLUDE
		...
		inline half CheckSame (float2 centerNormal, float centerDepth, float4 theSample)
		{
			// difference in normals
			// do not bother decoding normals - there's no need here
			float2 diff = abs(centerNormal - theSample.xy);
			int isSameNormal = (diff.x + diff.y)*0.5f < 0.005;
			return isSameNormal ? 1.0 : 0.0;
			// difference in depth
			float sampleDepth = DecodeFloatRG (theSample.zw);
			float zdiff = abs(centerDepth-sampleDepth);
			// scale the required threshold by the distance
			int isSameDepth = zdiff < 0.2 * centerDepth;
		...
		float4 FragRobert(Varyings i) : SV_Target
		{
			...
			float4 sample1 = tex2D(_CameraDepthNormalsTexture, i.texcoord[1].xy);
			float4 sample2 = tex2D(_CameraDepthNormalsTexture, i.texcoord[2].xy);
			float4 sample3 = tex2D(_CameraDepthNormalsTexture, i.texcoord[3].xy);
			float4 sample4 = tex2D(_CameraDepthNormalsTexture, i.texcoord[4].xy);
		...
		float4 FragThin(VaryingsLum i) : SV_Target
		{
			...
			float4 center = tex2D(_CameraDepthNormalsTexture, i.texcoord[0]);
			float4 sample1 = tex2D(_CameraDepthNormalsTexture, i.texcoord[1]);
			float4 sample2 = tex2D(_CameraDepthNormalsTexture, i.texcoord[2]);
			...
			half edge = 1.0;
			edge *= CheckSame(centerNormal, centerDepth, sample1) * CheckSame(centerNormal, centerDepth, sample2);
			
			return float4(center.r,center.g,0,1);
		}
